...
# This file is a script version of 'analyze-damp-pend.ipynb'
...
EXPERIMENT_DIR = './experiment-damp-pend/'
...
def get_model(args, baseline, structure, damping, num_points, gym=False, no_damp=False):
    if structure == False and baseline == True and no_damp == False:
    ...
    elif structure == False and baseline == False and no_damp == False:
    ...
    no_damp = '-noDamp' if no_damp else ''
    path = '{}pend-{}{}{}-{}-p{}{}.tar'.format(args.save_dir, model_name, struct, no_damp, args.solver, num_points, rad)
    model.load_state_dict(torch.load(path, map_location=device))
    path = '{}/pend-{}{}{}-{}-p{}-stats{}.pkl'.format(args.save_dir, model_name, struct, no_damp, args.solver, num_points, rad)
...
symoden_ode_struct_nodamp_model, symoden_ode_struct_nodamp_stats = get_model(args, baseline=False, structure=True, damping=False, num_points=args.num_points, gym=args.gym, no_damp=True)
...
print('')
print('SymODEN nodamp contains {} parameters'.format(get_model_parm_nums(symoden_ode_struct_nodamp_model)))
print('Final trajectory train loss {:.4e} +/- {:.4e}\nFinal trajectory test loss {:.4e} +/- {:.4e}'
.format(np.mean(symoden_ode_struct_nodamp_stats['traj_train_loss']), np.std(symoden_ode_struct_nodamp_stats['traj_train_loss']),
        np.mean(symoden_ode_struct_nodamp_stats['traj_test_loss']), np.std(symoden_ode_struct_nodamp_stats['traj_test_loss'])))
print('')
...
symoden_struct_nodamp_pred_loss = get_pred_loss(pred_x, pred_t_eval, symoden_ode_struct_nodamp_model)
...
print('')
print('SymODEN nodamp')
print('Prediction error {:.4e} +/- {:.4e}'
.format(np.mean(symoden_struct_nodamp_pred_loss), np.std(symoden_struct_nodamp_pred_loss)))
print('')
print('SymODEN')
print('Prediction error {:.4e} +/- {:.4e}'
...
symoden_struct_nodamp_ivp = integrate_model(symoden_ode_struct_nodamp_model, t_span, y0_u, **kwargs)
...
symoden_struct_nodamp_field = get_vector_field(symoden_ode_struct_nodamp_model, **kwargs)
...
#%%
fig = plt.figure(figsize=(12.8, 3.2), dpi=DPI)
plt.subplot(1, 4, 1)

x, y, t = get_trajectory(timesteps=200, noise_std=0.0, y0=y0, u=u0)

plt.plot(x,y, label='Ground Truth', color='k', linewidth=1)
plt.plot(base_ivp['y'][0,:],base_ivp['y'][1,:], 'r', label='Naive Baseline', linewidth=1.3)

plt.quiver(field['x'][:,0], field['x'][:,1], base_field[:,0], base_field[:,1],
        cmap='gray_r', scale=ARROW_SCALE, width=ARROW_WIDTH, color=(.5,.5,.5), alpha=0.4)

plt.xlabel("$q$", fontsize=14)
plt.ylabel("$p$", rotation=0, fontsize=14)
plt.title("Naive Baseline", pad=10)
plt.xlim(-R, R)
plt.ylim(-R, R)
...
plt.plot(x,y, label='Ground Truth', color='k', linewidth=1)
plt.plot(symoden_ivp['y'][0,:],symoden_ivp['y'][1,:], 'r', label='Unstr. Dissipative SymODEN', linewidth=1.3)
...
plt.xlabel("$q$", fontsize=14)
plt.ylabel("$p$", rotation=0, fontsize=14)
plt.title("Unstr. Dissipative SymODEN", pad=10)
plt.xlim(-R, R)
plt.ylim(-R, R)
# plt.legend(fontsize=10)
...
plt.plot(x,y, label='Ground Truth', color='k', linewidth=1)
plt.plot(symoden_struct_nodamp_ivp['y'][0,:],symoden_struct_nodamp_ivp['y'][1,:], 'r', label='SymODEN', linewidth=1.3)

plt.quiver(field['x'][:,0], field['x'][:,1], symoden_struct_nodamp_field[:,0], symoden_struct_nodamp_field[:,1],
        cmap='gray_r', scale=ARROW_SCALE, width=ARROW_WIDTH, color=(.5,.5,.5), alpha=0.4)

plt.xlabel("$q$", fontsize=14)
plt.ylabel("$p$", rotation=0, fontsize=14)
plt.title("SymODEN", pad=10)
plt.xlim(-R, R)
plt.ylim(-R, R)
# plt.legend(fontsize=10)
...
plt.plot(x,y, label='Ground Truth', color='k', linewidth=1)
plt.plot(symoden_struct_ivp['y'][0,:],symoden_struct_ivp['y'][1,:], 'r', label='Dissipative SymODEN', linewidth=1.3)
...
plt.xlabel("$q$", fontsize=14)
plt.ylabel("$p$", rotation=0, fontsize=14)
plt.title("Dissipative SymODEN", pad=10)
plt.xlim(-R, R)
plt.ylim(-R, R)
# plt.legend(fontsize=10)

plt.tight_layout()
fig.savefig('{}/fig-damp-pend-traj.{}'.format(args.fig_dir, FORMAT))
...
plt.plot(q, g_q.detach().cpu().numpy(), 'b--', linewidth=3, label=r'Dissipative SymODEN $g_{\theta_3}(q)$')
plt.xlabel("$q$", fontsize=14)
...
plt.plot(q, M_q_inv.detach().cpu().numpy(), 'b--', linewidth=3, label=r'Dissipative SymODEN $M^{-1}_{\theta_1}(q)$')
plt.xlabel("$q$", fontsize=14)
...
plt.plot(q, V_q.detach().cpu().numpy(), 'b--', linewidth=3, label=r'Dissipative SymODEN $V_{\theta_2}(q)$')
plt.xlabel("$q$", fontsize=14)
...
plt.legend(fontsize=9.5)
plt.tight_layout()
# fig.savefig('{}/fig-damp-pend.{}'.format(args.fig_dir, FORMAT))
...
damp_matrix = symoden_ode_struct_model.damp_net(q_tensor)

#%%
fig = plt.figure(figsize=(9.6, 2.5), dpi=DPI)

plt.subplot(1, 3, 1)
plt.plot(q, 0 * np.ones_like(q), label='Ground Truth', color='k', linewidth=2)
plt.plot(q, damp_matrix[:,0,0].detach().cpu().numpy(), 'b--', linewidth=3, label=r'Dissipative SymODEN $D_{0,0}$')
plt.xlabel("$q$", fontsize=14)
# plt.ylabel("$g(q)$", rotation=0, fontsize=14)
plt.title(r"$D_{0,0}$", pad=10, fontsize=14)
plt.xlim(-5, 5)
plt.ylim(-0.2, 0.2)
plt.legend(fontsize=9.5)
...
plt.subplot(1, 3, 2)
plt.plot(q, 0 * np.ones_like(q), label='Ground Truth', color='k', linewidth=2)
plt.plot(q, damp_matrix[:,0,1].detach().cpu().numpy(), 'b--', linewidth=3, label=r'Dissipative SymODEN $D_{0, 1}$')
plt.xlabel("$q$", fontsize=14)
# plt.ylabel("$M^{-1}(q)$", rotation=0, fontsize=14)
plt.title(r"$D_{0,1} (= D_{1,0})$", pad=10, fontsize=14)
plt.xlim(-5, 5)
plt.ylim(-0.2, 0.2)
plt.legend(fontsize=9.5)

plt.subplot(1, 3, 3)
plt.plot(q, 0.1 * np.ones_like(q), label='Ground Truth', color='k', linewidth=2)
plt.plot(q, damp_matrix[:,1,1].detach().cpu().numpy(), 'b--', linewidth=3, label=r'Dissipative SymODEN $D_{1, 1}$')
plt.xlabel("$q$", fontsize=14)
# plt.ylabel("$V(q)$", rotation=0, fontsize=14)
plt.title(r"$D_{1,1}$", pad=10, fontsize=14)
plt.xlim(-5, 5)
plt.ylim(-0.2, 0.2)
plt.legend(fontsize=9.5)
plt.tight_layout()
# fig.savefig('{}/fig-damp-pend-matrix.{}'.format(args.fig_dir, FORMAT))
