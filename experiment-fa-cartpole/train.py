...
def get_args():
    ...
    parser.add_argument('--name', default='fa_cart', type=str, help='only one option right now')
...
def train(args):
    ...
    if args.structure == False:
        if args.naive and args.baseline:
            ...
            model = SymODEN_R1_T1(args.num_angle, H_net=nn_model, device=device, baseline=args.baseline, naive=args.naive, u_dim=2)
        elif args.baseline:
            input_dim = 6
            output_dim = 4
            nn_model = MLP(input_dim, 700, output_dim, args.nonlinearity).to(device)
            model = SymODEN_R1_T1(args.num_angle, H_net=nn_model, M_net=M_net, device=device, baseline=args.baseline, naive=args.naive, u_dim=2)
        else:
            input_dim = 5
            output_dim = 1
            nn_model = MLP(input_dim, 500, output_dim, args.nonlinearity).to(device)
            model = SymODEN_R1_T1(args.num_angle, H_net=nn_model, M_net=M_net, g_net=g_net, device=device, baseline=args.baseline, naive=args.naive, u_dim=2)
    elif args.structure == True and args.baseline ==False and args.naive==False:
        V_net = MLP(3, 300, 1).to(device)
        model = SymODEN_R1_T1(args.num_angle, M_net=M_net, V_net=V_net, g_net=g_net, device=device, baseline=args.baseline, structure=True, u_dim=2).to(device)
    else:
