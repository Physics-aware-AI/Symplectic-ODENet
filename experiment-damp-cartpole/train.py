...
def train(args):
    ...
    if args.structure == False:
        if args.naive and args.baseline:
            ...
            model = SymODEN_R1_T1(args.num_angle, H_net=nn_model, M_net=M_net, g_net=g_net, device=device, baseline=args.baseline, naive=args.naive, damp_net=damp_net).to(device)
        ...
        if args.no_damp:
            ...
            damp_net = PSD(3, 100, 4, eps=0.0)
            model = SymODEN_R1_T1(args.num_angle, M_net=M_net, V_net=V_net, g_net=g_net, device=device, baseline=args.baseline, structure=True, damp_net=damp_net).to(device)
