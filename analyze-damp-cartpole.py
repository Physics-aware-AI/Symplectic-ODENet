...
# This file is a script version of 'analyze-damp-cartpole.ipynb'
...
EXPERIMENT_DIR = './experiment-damp-cartpole/'
...
def get_model(args, baseline, structure, naive, damping, num_points, no_damp=False):
    ...
    if structure == False:
        if naive and baseline:
            ...
            input_dim = 5
            output_dim = 1
            damp_net = PSD(3, 100, 4, eps=0.0)
            nn_model = MLP(input_dim, 500, output_dim, args.nonlinearity)
            model = SymODEN_R1_T1(args.num_angle, H_net=nn_model, M_net=M_net, g_net=g_net, device=device, baseline=baseline, naive=naive, damp_net=damp_net).to(device)
        ...
        if no_damp:
            ...
            damp_net = PSD(3, 100, 4, eps=0.0)
            model = SymODEN_R1_T1(args.num_angle, M_net=M_net, V_net=V_net, g_net=g_net, device=device, baseline=baseline, structure=True, damp_net=damp_net).to(device)
    else:
...
env = gym.make('MyDampCartPole-v0')
