...
from hnn import HNN, HNN_structure_forcing, HNN_structure_embed, HNN_structure_cart_embed
...
def integrate_models(y0=np.asarray([1, 0]), u0=0.0, t_span=[0,5], t_eval=None):
    ...
    hnn_path = integrate_model(hnn_ode_model, t_span, y0_u, **kwargs)
    hnn_x = hnn_path['y'].T

    hnn_struct_path = integrate_model(hnn_ode_struct_model, t_span, y0_u, **kwargs)
...
true_x_single, base_x_single, hnn_x_single, hnn_struct_x_single = integrate_models(y0=y0, u0=u0, t_span=t_span, t_eval=t_eval_single)
...
E_hnn_single = get_energy(hnn_x_single)
E_hnn_struct_single = get_energy(hnn_struct_x_single)
...
def integrate_models(y0=np.asarray([1, 0]), u0=0.0, t_span=[0,5], t_eval=None):
    ...
    # integrate along HNN structure vector field
    hnn_struct_path = integrate_model(hnn_ode_struct_model, t_span, y0_u, **kwargs)
    hnn_struct_x = hnn_struct_path['y'].T

    return naive_x, base_x, hnn_x, hnn_struct_x
...
u0 = 0.0
naive_x_embed, base_x_embed, hnn_x_embed, hnn_struct_x_embed = integrate_models(y0=y0, u0=u0, t_span=t_span, t_eval=t_eval_embed)
...
E_hnn_embed = get_energy(hnn_x_embed)
E_hnn_struct_embed = get_energy(hnn_struct_x_embed)
...
def integrate_model(model, t_span, y0, **kwargs):
    ...
    return solve_ivp(fun=fun, t_span=t_span, y0=y0, **kwargs)
naive_x_cart, base_x_cart, hnn_x_cart, hnn_struct_x_cart = integrate_models(y0=y0, u0=u0, t_span=t_span, t_eval=t_eval_cart)
...
E_hnn_cart = get_energy(hnn_x_cart)
E_hnn_struct_cart = get_energy(hnn_struct_x_cart)
...
naive_x_double, base_x_double, hnn_x_double, hnn_struct_x_double = integrate_models(y0=y0, u0=u0, t_span=t_span, t_eval=t_eval_double)
...
E_hnn_double = get_energy(hnn_x_double)
E_hnn_struct_double = get_energy(hnn_struct_x_double)
#%%
fig = plt.figure(figsize=(12, 4.8), dpi=DPI)
plt.subplot(2, 4, 1)
plt.plot(t_eval_single, ((true_x_single-base_x_single)**2).mean(-1), 'y', linewidth=2)
plt.plot(t_eval_single, ((true_x_single-hnn_x_single)**2).mean(-1), 'g', linewidth=2)
plt.plot(t_eval_single, ((true_x_single-hnn_struct_x_single)**2).mean(-1), 'b', linewidth=2)
plt.ylim(-0.5, 10)
plt.xlabel('t')
plt.ylabel('MSE')
...
plt.plot(t_eval_embed, ((true_x_embed-hnn_x_embed)**2).mean(-1), 'g', linewidth=2)
plt.plot(t_eval_embed, ((true_x_embed-hnn_struct_x_embed)**2).mean(-1), 'b', linewidth=2)
plt.ylim(-0.5, 8)
plt.xlabel('t')
...
plt.plot(t_eval_cart, ((true_x_cart-hnn_x_cart)**2).mean(-1), 'g', linewidth=2)
plt.plot(t_eval_cart, ((true_x_cart-hnn_struct_x_cart)**2).mean(-1), 'b', linewidth=2)
plt.ylim(-4, 70)
plt.xlabel('t')
...
plt.plot(t_eval_double, ((true_x_double-hnn_x_double)**2).mean(-1), 'g', label='Unstructured SymODEN', linewidth=2)
plt.plot(t_eval_double, ((true_x_double-hnn_struct_x_double)**2).mean(-1), 'b', label='SymODEN', linewidth=2)
# plt.ylim(-1, 40)
plt.xlabel('t')
...
plt.plot(t_eval_single, E_hnn_single, 'g', linewidth=2)
plt.plot(t_eval_single, E_hnn_struct_single, 'b', linewidth=2)
plt.ylim(6, 12)
plt.xlabel('t')
...
plt.plot(t_eval_embed, E_hnn_embed, 'g', linewidth=2)
plt.plot(t_eval_embed, E_hnn_struct_embed, 'b', linewidth=2)
plt.ylim(0.40, 0.8)
plt.xlabel('t')
# plt.legend(fontsize=6)
...
plt.plot(t_eval_cart, E_hnn_cart, 'g', linewidth=2)
plt.plot(t_eval_cart, E_hnn_struct_cart, 'b', linewidth=2)
plt.ylim(-3, 7)
plt.xlabel('t')
# plt.legend(fontsize=6)
...
plt.plot(t_eval_double, E_hnn_double, 'g', label='Unstructured SymODEN', linewidth=2)
plt.plot(t_eval_double, E_hnn_struct_double, 'b', label='SymODEN', linewidth=2)
# plt.ylim(-3, 3)
plt.xlabel('t')
plt.legend(fontsize=7)
