...
EXPERIMENT_DIR = './experiment-damp-pend-embed/'
...
def get_model(args, baseline, structure, naive, damping, num_points, no_damp=False):
    ...
    if structure == False:
        if naive and baseline:
            ...
            nn_model = MLP(input_dim, 500, output_dim, args.nonlinearity)
            model = SymODEN_T(args.num_angle, H_net=nn_model, M_net=M_net, g_net=g_net, device=device, baseline=baseline, naive=naive, damp_net=damp_net).to(device)
        ...
        if no_damp:
            ...
            damp_net = PSD(2*args.num_angle, 100, 2*args.num_angle, eps=0.0)
            model = SymODEN_T(args.num_angle, M_net=M_net, V_net=V_net, g_net=g_net, device=device, baseline=baseline, structure=True, damp_net=damp_net).to(device)
    else:
...
symoden_ode_struct_nodamp_model, symoden_ode_struct_nodamp_stats = get_model(args, baseline=False, structure=True, naive=False, damping=False, num_points=args.num_points, no_damp=True)
...
import gym 
import myenv
env = gym.make('MyDampPendulum-v0')
env.reset()
...
symoden_struct_nodamp_qp = get_qp(symoden_struct_nodamp_ivp.y.T)
...
symoden_struct_nodamp_1 = symoden_struct_nodamp_ivp.y[0,:]**2 + symoden_struct_nodamp_ivp.y[1,:]**2
symoden_struct_1 = symoden_struct_ivp.y[0,:]**2 + symoden_struct_ivp.y[1,:]**2
...
plt.plot(t_linspace_model, symoden_struct_nodamp_1, 'c', label='SymODEN')
plt.plot(t_linspace_model, symoden_struct_1, 'b', label='Damp SymODEN')
...
plt.plot(q, beta * g_q.detach().cpu().numpy(), 'b--', linewidth=3, label=r'Dissipative SymODEN $\beta g_{\theta_3}(q)$')
...
plt.plot(q, M_q_inv.detach().cpu().numpy()/beta, 'b--', linewidth=3, label=r'Dissipative SymODEN $M^{-1}_{\theta_1}(q)/\beta$')
plt.xlabel("$q$", fontsize=14)
...
plt.plot(q, beta * V_q.detach().cpu().numpy(), 'b--', label=r'Dissipative SymODEN $\beta V_{\theta_2}(q)$', linewidth=3)
plt.xlabel("$q$", fontsize=14)
...
plt.legend(fontsize=9)
plt.tight_layout()
# fig.savefig('{}/fig-damp-pend-embed.{}'.format(args.fig_dir, FORMAT))

#%%

damp_matrix = symoden_ode_struct_model.damp_net(cos_q_sin_q)
fig = plt.figure(figsize=(9.6, 2.5), dpi=DPI)
...
plt.subplot(1, 3, 3)
plt.plot(q, 0.932*0.3 * np.ones_like(q), label='Ground Truth', color='k', linewidth=2)
...
plt.legend(fontsize=9.5)
plt.tight_layout()
# fig.savefig('{}/fig-damp-pend-embed-matrix.{}'.format(args.fig_dir, FORMAT))
